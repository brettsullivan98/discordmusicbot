...
async def continue_queue(ctx):
    ...
    if not session.q.theres_next():
        await ctx.send("finished queue go jerk off now")
        return
    ...
    await ctx.send(f"playing some shit song called: {session.q.current_music.title}")
...
async def play(ctx, *, arg):
    ...
    if voice.is_playing():
        ...
        await ctx.send(f"added shit song to queue: {title}")
        return
    else:
        await ctx.send(thumb)
...
async def skip(ctx):
    ...
    if not session.q.theres_next():
        await ctx.send("no song to be played")
        return
...
async def print_info(ctx):
    ...
    await ctx.send(f"current shitty song is: {session.q.current_music.title}")
...
async def resume(ctx):
    ...
    if voice.is_paused:
        ...
        await ctx.send("music is already paused dumbass")
...
async def stop(ctx):
    ...
    if voice.is_playing:
        ...
        await ctx.send("theres nothing playing fool")
